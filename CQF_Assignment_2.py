...
def test_closed_form_solutions(spot_space,
    ...
    for sp_idx, sp in enumerate(spot_space, 0):
        for ts_idx, ts in enumerate(time_space, 0):
            ...
            my_lookback = ClosedFormContinuousLookback(sp, option_type, strike_type, risk_free_rate, ts, asset_volatility, strike_price, sp, sp)

            option_prices[0, 0, sp_idx, ts_idx] = my_black_scholes.call_price
            option_prices[1, 0, sp_idx, ts_idx] = my_geometric_asian.call_price
            option_prices[2, 0, sp_idx, ts_idx] = my_lookback.call_price
            option_prices[0, 1, sp_idx, ts_idx] = my_black_scholes.put_price
            option_prices[1, 1, sp_idx, ts_idx] = my_geometric_asian.put_price
            option_prices[2, 1, sp_idx, ts_idx] = my_lookback.put_price
        
        
    return option_prices

    
def test_monte_carlo_payoffs(spot_space,
                             time_space,
                             option_type,
                             strike_type,
                             averaging_type,
    ...
    for sp_idx, sp in enumerate(spot_space, 0):
        for ts_idx, ts in enumerate(time_space, 0):
            ...
            my_asian = AsianOption(asset_paths, option_type, strike_type, averaging_type, strike_price, risk_free_rate, ts)
            my_european = EuropeanOption(asset_paths, option_type, strike_price, risk_free_rate, ts)
            my_lookback = LookbackOption(asset_paths, option_type, strike_type, strike_price, risk_free_rate, ts)
                
            option_prices[0, 0, sp_idx, ts_idx] = my_european.call_price
            option_prices[1, 0, sp_idx, ts_idx] = my_asian.call_price
            option_prices[2, 0, sp_idx, ts_idx] = my_lookback.call_price
            option_prices[0, 1, sp_idx, ts_idx] = my_european.put_price
            option_prices[1, 1, sp_idx, ts_idx] = my_asian.put_price
            option_prices[2, 1, sp_idx, ts_idx] = my_lookback.put_price
...
# Utilities
def root_mean_squared_error(option_prices_method_1, option_prices_method_2):
    
    call_rmse = np.sqrt(np.sum(np.sum(np.power(option_prices_method_1[0] - option_prices_method_2[0], 2), axis = 0)))
    put_rmse = np.sqrt(np.sum(np.sum(np.power(option_prices_method_1[1] - option_prices_method_2[1], 2), axis = 0)))
    return call_rmse, put_rmse


def visualize_line_plot(vol_or_time_flag, option_type, strike_type, x_var, y_var, z_var):
...
def visualize_grid_plot(option_prices, spot_space, time_space):
    
    european_surface = pd.DataFrame(option_prices[0][0], index= spot_space, columns= time_space,)
    ...
    asian_surface = pd.DataFrame(option_prices[1][0], index= spot_space, columns= time_space,)
    ...
    lookback_surface = pd.DataFrame(option_prices[2][0], index= spot_space, columns= time_space,)
